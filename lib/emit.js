...
const updateContext = (t, val, stamp, parent, prev, j, fn, contextFix, level) => {
  var clear
  if (parent.instances) {
    let key = prev
    ...
    while (i--) {
      if (!(key in parent.instances[i])) {
        ...
        if (contextFix) {
          ...
          contextFix.contextLevel = level
        } else {
          t.context = context
          t.contextLevel = level
        }
  ...
  if (parent.parent) {
    if (updateContext(t, val, stamp, parent.parent, parent.key, j, fn, contextFix, level + 1)) {
      clear = true
    }
  }
  return clear
...
const fn = (t, val, stamp, emitters) => {
  ...
  if (listeners) {
    ...
    if (i && t.parent) {
      if (updateContext(t, val, stamp, t.parent, t.key, i, listeners, 1)) {
        delete t.context
