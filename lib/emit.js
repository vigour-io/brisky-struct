...
const updateContext = (context, t, val, stamp, key, resolve, level, j, fn) => {
  if (!(key in context)) {
    ...
    // SOME CASES YOU NEED THIS
    // but how to figure out that a parent of instance is the same?
    if (context._p) {
      // this can be double
      const nr = (Math.random() * 100) | 0
      console.log('1 - HERE', nr, context._k, level)
      if (emitContext(t, val, stamp, context._p, context._k, context, 1, j, fn)) {
        console.log('1!', nr)
...
const emitContext = (t, val, stamp, parent, key, resolve, level, j, fn) => {
  ...
  if (parent._p) {
    // this can be double
    const nr = (Math.random() * 100) | 0
    console.log('2 - HERE', nr, t._k, level)
    if (emitContext(t, val, stamp, parent._p, parent._k, resolve, level + 1, j, fn)) {
      console.log('2!', nr)
      clear = true
    }
  }
...
const fn = (t, val, stamp, emitter) => {
  ...
  if (listeners) {
    ...
    if (i && t._p) {
      if (emitContext(t, val, stamp, t._p, t._k, t, 1, i, listeners)) {
        t.context = null
        t.contextPath = null
      }
    }
