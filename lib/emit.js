...
const emitContextInstance = (instance, t, val, stamp, key, resolve, level, j, fn) => {
  var clear
  if (!(key in instance)) {
    let z = j
    ...
    if (instance.instances) {
      let i = instance.instances.length
      while (i--) { emitContextInstance(instance.instances[i], t, val, stamp, key, resolve, level, j, fn) }
    }
  }
  return clear
}
