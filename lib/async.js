...
const queue = t => {
  ...
  if (async && async.next) {
    execIterator(t, async, t.async[1], done)
  } else if (!t.async[2]) {
    execPromise(t)
  }
}
...
const execIterator = (t, iteratee, stamp, done, val) => {
  if (!val || !val.done) {
    if (val !== void 0) {
      if (
        val.value &&
        ...
        val.value.then((resolved) => {
          ...
          execIterator(t, iteratee, stamp, done, iteratee.next())
...
const iterator = (t, iteratee, stamp, val) => {
  if (!t.async) {
    t.async = [ iteratee, stamp, void 0 ]
    queue(t)
  } else {
    t.async.push(iteratee, stamp, void 0)
  }
}
