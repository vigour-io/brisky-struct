...
var set = (t, val, stamp, isNew) => {
  ...
  if (changed) {
    ...
    if (stamp !== t.tStamp && val && typeof val === 'object' && val.stamp) {
      // not changed no emitters -- needs tests
      // only subscription nothing else no listeners!
      console.log('go!', t.path())
      data(t, val, stamp, isNew)
      // way too much allways happens if a hub sends you same stuff again
    }
  }
  return changed
}
