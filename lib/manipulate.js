  ...
  if (changed === true) {
    ...
    while (--i > -1) {
      if (!t.instances[i].keys) {
        ...
        while (--j > -1) {
          if (changed[j] in t.instances[i]) {
            console.log('has its own skip', changed[j], t.instances[i].key)
          } else {
            needupdate = true
    ...
    if (type === 'object' && type !== 'function') {
      if (!val) {
        ...
        if (val.inherits) {
          ...
          // wastefull check can be shared with the bottom
          if (t.instances) {
            for (let k in val) {
              if (k !== 'val'
