...
const create = (t, val, stamp, parent, key) => {
  ...
  if (parent) {
    if (val && typeof val === 'object' && val.type) {
      // here we get type on create need to deal with it if the original is of a different type...
      // why not just ignore anythign else then switching? -- this prob needs to do stuff as well
      // this needs to merge -- especialy in context etc...
      instance = createType(parent, val, t, stamp) || t
    } else {
  ...
  if (val.val === null) {
    ...
    if (t.instances) {
      ...
      for (let key in val) {
        if (key !== 'stamp') {
          let result = key !== 'val'
              ? getProp(t, key)(t, val[key], key, stamp, isNew)
  ...
  if (val.stamp) {
    ...
    if (changed) {
      if (stamp) { data(t, val, stamp, false, isNew) }
        // this is the issue need to know if it actually added a key,,,
