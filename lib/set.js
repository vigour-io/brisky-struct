...
const child = (target, key) => target.child || child(target.inherits)
const defintion = (target, key) => target.properties
  ? target.properties[key]
  : defintion(target.inherits, key)

const property = (target, key, val, stamp) => {
  const custom = defintion(target, key)
  var changed
  if (custom) {
    ...
    if (result) {
      ...
      changed = true
      addKey(target, key)
      const result = create(child(target), val, stamp)
      result.key = key
