  ...
  if (key in t) {
    ...
    if (result) {
      if (result !== val) {
        ...
        if (val === null) {
          t[key] = null
          console.log('REMOVE --- needs to remove from ARRAY', key)
          // not enough...
        } else {
          t[key] = val
        }
      ...
      if (val === null) {
        console.error('!!!ALLREADY REMOVED', t.struct) // something to do with get
      } else {
        add(t, val, key)
      }
    }
    ...
    if (result && typeof result === 'function') {
      if (result !== val) {
        if (val !== null && typeof val !== 'function') {
          throw new Error('STRUCT CONTEXT WRONG')
        }
