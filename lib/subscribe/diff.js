...
const diff = (t, subs, cb, tree, removed, referenced) => {
  var changed
  for (let key in subs) {
    if (key !== 'val' && key !== '$' && key !== '_' && key !== '$remove') {
      if (item(key, t, subs[key], cb, tree, removed, referenced)) {
        changed = true
      }
    }
  }
...
const item = (key, t, subs, cb, tree, removed, referenced) => {
  if (key === 'val') {
  ...
  var changed
  if (key === '$any') {
    any(t, subs, cb, tree, removed, referenced)
  } else {
