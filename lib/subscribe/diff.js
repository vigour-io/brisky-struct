...
const prev = (t, previous, key, referenced) => {
  // console.log(referenced.path(), previous.path())
  // console.log(referenced.val.path())
  var block
  ...
  // if (block) {
    while (previous) {
      if (previous === t) {
        console.log('need to check if previous had problems')
        // exclude(previous, key)

        // hard case -- need to check that preivous is not referenced anymore
    ...
    if (!referenced || removed || exclude(referenced, key, t)) {
      if (removed) {
        ...
        changed = property(
          ...
          cb,
          tree,
          previous && prev(t, previous, key, referenced)
        )
      }
    }
  }
