...
const g = require('../keys').getKeys

const backCheck = (key, t, index) => {
  var i = 0
    ...
    while (t && typeof t === 'object' && t.inherits) {
      ...
      if (i) {
        if (!combined) {
          if (keys) {
            combined = []
            for (let j = 0, len = keys.length; i < len; i++) {
              combined[j] = keys[j]
            }
          } else {
            combined = []
...
const create = (keys, t, subs, cb, tree) => {
  ...
  const branch = tree.$any = { $t: t, _p: tree, _key: '$any', $keys }
  for (let i = 0; i < len; i++) {
    let key = keys[i]
    $keys[i] = key
...
const single = (key, i, t, subs, cb, branch) => {
  return property(key, t, subs, cb, branch)
}
...
const update = (keys, t, subs, cb, tree) => {
  ...
  for (let i = 0; i < len; i++) {
    ...
    let compare = $keys[i]
    if (key === compare) {
      changed = single(key, i, t, subs, cb, branch)
    } else {
      if (!hot) {
        hot = [ key, compare, i ]
      } else {
        ...
        while (!block && (j -= 3) > -1) {
          if (key !== void 0 && hot[j + 1] === key) {
            ...
            let index = hot[j + 2]
            $keys[index] = compare
            changed = single(compare, index, t, subs, cb, branch)
            if (key === hot[j + 1]) {
              hot.splice(j, 3)
              block = true
            } else {
