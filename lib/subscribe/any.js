  ...
  if (removed || !t) {
    ...
    if (keys) {
      if (!tree.$any) {
        ...
        create(keys, t, subs, cb, tree, previous)
        return true
      } else {
        return update(keys, t, subs, cb, tree, previous)
      }
    }
  }
}
...
const create = (keys, t, subs, cb, tree, previous) => {
  const len = keys.length
  ...
  for (let i = 0; i < len; i++) {
    let key = keys[i]
    $keys[i] = key
    item(key, t, subs, cb, branch, void 0, void 0, previous)
    $m[i] = branch[key].$
  }
}

const single = (key, i, $m, t, subs, cb, branch, previous) => {
  let k = t && get(t, key)
  // composite
  // will recreate stuff from item in here (item prop or something less complex)
  if ($m[i] !== k.tStamp || 0) {
    if (item(key, t, subs, cb, branch, void 0, void 0, previous)) {
      $m[i] = branch[key].$
      return true
    }
  }
...
const modify = (hot, $keys, $m, t, subs, cb, branch, previous) => {
  for (let i = 0, len = hot.length; i < len - 3; i += 4) {
    ...
    if (create) {
      ...
      item(create, t, subs, cb, branch, void 0, void 0, previous)
      $keys[index] = create
...
const update = (keys, t, subs, cb, tree, previous) => {
  var hot, changed
  const branch = tree.$any
  const $keys = branch.$keys
  const $m = branch.$m
  ...
  for (let i = 0; i < len; i++) {
    ...
    if (key === compare) {
      changed = single(key, i, $m, t, subs, cb, branch, previous)
    } else {
      let mem = $m[i]
      // $keys[i] = key
      if (!hot) {
        ...
        while (!block && (j -= 4) > -1) {
          if (key !== void 0 && hot[j + 1] === key) {
            ...
            $keys[index] = compare
            $m[index] = mem
            changed = single(compare, index, $m, t, subs, cb, branch, previous)
            if (key === hot[j + 1]) {
  ...
  if (hot) {
    modify(hot, $keys, $m, t, subs, cb, branch, previous)
    changed = true
  }
  return changed
}
