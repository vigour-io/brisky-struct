...
module.exports = (t, subs, cb, tree, referenced, removed, previous) => {
  ...
  if (removed || !t) {
    if (tree.$any) {
      ...
      removeFields(t, subs, cb, tree, void 0, removed, previous)
      return true
    }
  } else {
    ...
    if (keys) {
      if (!tree.$any) {
        ...
        create(keys, t, subs, cb, tree, void 0, previous)
        return true
      } else {
...
const removeFields = (t, subs, cb, tree, referenced, previous) => {
  const branch = tree.$any
  const $keys = branch.$keys
  const len = $keys.length
  for (let i = 0; i < len; i++) {
    item($keys[i], t, subs, cb, branch, referenced, true, previous)
  }
  delete tree.$any
}
