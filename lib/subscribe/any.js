...
const update = (key, keys, t, subs, cb, branch) => {
  ...
  for (let i = 0; i < len2; i++) {
    ...
    // $keys[i] // = key
    let tt = getOrigin(t, key)
    // console.log('???', key, i, tt.path(), 'vs', branch.$keys[i].$t.path())
    // need to pass the branch straight will ne a lot faster

    // if not the same, etc
    updateProperty(i, tt, subs, cb, branch.$keys)
  }
}
