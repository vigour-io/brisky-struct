...
const swap = (key, current, newIndex, $keys, $m, t, subs, cb, branch, referenced, previous) => {
  $keys[newIndex] = key
  $m[newIndex] = $m[current]
  // console.log('  go update key', key, $m[current])
  if (key) {
    return singleUpdate(key, newIndex, $m, t, subs, cb, branch, referenced, previous)
  }
}
      ...
      if (!hot) {
        ...
        while (!block && (j -= 3) > -1) {
          if (hot[j + 1] === key) {
            ...
            changed = swap(key, hot[j + 2], i, $keys, $m, t, subs, cb, branch, referenced, previous)
            if (compareKey === hot[j]) {
              hot.splice(j, 3)
              block = true
            } else {
            ...
            // current old -> new 1 -> 0
            changed = swap(compareKey, i, hot[j + 2], $keys, $m, t, subs, cb, branch, referenced, previous)
            if (key === hot[j + 1]) {
              hot.splice(j, 3)
              block = true
            } else {
...
const update = (keys, t, subs, cb, tree, referenced, previous) => {
  ...
  console.log(len1 > len2 ? '$keys is larger' : 'keys is larger')
