...
const removeFields = (t, subs, cb, tree, referenced) => {
  ...
  for (let i = 0; i < len; i++) {
    property($keys[i], t, subs, cb, branch, true, referenced)
  }
  delete tree.$any
}
...
const create = (keys, t, subs, cb, tree, referenced, previous) => {
  ...
  for (let i = 0; i < len; i++) {
    let key = keys[i]
    $keys[i] = key
    property(key, t, subs, cb, branch, void 0, referenced, previous)
    if (branch[key]) {
      $m[i] = branch[key].$
    }
  }
...
const single = (key, i, $m, t, subs, cb, branch, referenced, previous) => {
  let k = t && get(t, key)
  if ($m[i] !== k.tStamp || 0) {
    if (property(key, t, subs, cb, branch, void 0, referenced, previous)) {
      $m[i] = branch[key].$
      return true
    }
  }
...
const modify = (hot, $keys, $m, t, subs, cb, branch, referenced, previous) => {
  for (let i = 0, len = hot.length; i < len - 3; i += 4) {
    ...
    if (remove) {
      ...
      property(remove, t, subs, cb, branch, true, referenced)
    ...
    if (create) {
      ...
      property(create, t, subs, cb, branch, void 0, referenced, previous)
      $keys[index] = create
