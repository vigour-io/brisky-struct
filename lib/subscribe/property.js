...
const updateReference = (t, subs, cb, tree) => {
  if (isStruct(t)) {
    if (tree.val && tree.val.$t !== t.val) {
      const old = tree.val
      console.log('here!', t.val)
      removeReference(subs, cb, tree.val, t.val, old)
...
const createReference = (t, subs, cb, tree, previous) => {
  if (isStruct(t)) { reference(t.val, subs, cb, tree, t, previous) }
}
...
const reference = (t, subs, cb, tree, referenced, previous) => {
  ...
  if (t && (t.val === void 0 || t.val !== null)) {
    ...
    if (!branch) {
      ...
      createReference(t, subs, cb, branch, previous)
      changed = true
