'use strict'
const { diff } = require('./diff')
const { remove, removeReference } = require('./remove')

const composite = () => {
  // changed = composite(travelt, subs, update, treeKey, stamp, void 0, removed)
}

const isStruct = t => t.val && typeof t.val === 'object' && t.val.inherits

const copy = (a) => {
  var r = {}
  for (var i in a) {
    if (i !== '_' && i !== '_p') {
      if (typeof a[i] === 'object' && i !== '$t') {
        r[i] = copy(a[i])
      } else {
        r[i] = a[i]
      }
    }
  }
  return r
}

const updateReference = (t, subs, cb, tree, referenced) => {
  if (isStruct(t)) {
    if (tree.val && tree.val.$t !== t.val) {
      // bit strange...
      if (tree.val.$t === t.val.origin()) {
        console.log(' \n!!!this goes wrong!!!')
      }
      console.log('change dat fucker')
      removeReference(subs, cb, tree.val) // maybe not even nessecary dont need to cancel
      console.log('removed dat fucker')
      reference(t.val, subs, cb, tree, referenced)
    } else {
      reference(t.val, subs, cb, tree, referenced)
    }
  } else if (tree.val) {
    removeReference(subs, cb, tree.val)
  }
}

const createReference = (t, subs, cb, tree, referenced) => {
  if (isStruct(t)) {
    reference(t.val, subs, cb, tree, referenced)
  }
}

const reference = (t, subs, cb, tree, referenced) => {
  var branch = tree.val
  var changed
  if (t) {
    const stamp = t.tStamp || t.stamp || 0
    if (!branch) {
      branch = tree.val = { _p: tree, _key: 'val', $t: t }
      branch.$ = stamp
      diff(t, subs, cb, branch, referenced)
      createReference(t, subs, cb, branch, referenced)
      changed = true
    } else if (branch.$ !== stamp) {
      branch.$ = stamp
      diff(t, subs, cb, branch, referenced)
      updateReference(t, subs, cb, branch, referenced)
      changed = true
    } else if (branch.$c) {
      changed = composite()
    }
  } else if (branch) {
    remove(subs, cb, branch)
    changed = true
  }
  return changed
}

const property = (key, t, subs, cb, tree) => {
  var branch = tree[key]
  var changed
  if (t) {
    const stamp = t.tStamp || 0
    if (!branch) {
      branch = tree[key] = { _p: tree, _key: key, $t: t }
      branch.$ = stamp
      if (tree.val) { refCreateFieldFinder(tree.val, key, subs, cb) }
      if (subs.val) { cb(t, 'new', subs, branch) }
      diff(t, subs, cb, branch)
      createReference(t, subs, cb, branch, t)
      changed = true
    } else if (branch.$ !== stamp) {
      branch.$ = stamp
      if (subs.val === true) { cb(t, 'update', subs, branch) }
      diff(t, subs, cb, branch)
      updateReference(t, subs, cb, branch, t)
      changed = true
    } else if (branch.$c) {
      changed = composite()
      if (changed && subs.val === true) { cb(t, 'update', subs, branch) }
    }
  } else if (branch) {
    remove(subs, cb, branch)
    if (tree.val) { refRemoveFieldFinder(tree.val, key, subs, cb) }
    changed = true
  }
  return changed
}

module.exports = property

// could use previous to set an update and ighnore rmeoves...
const refRemoveFieldFinder = (branch, key, subs, cb) => {
  const t = branch.$t.get(key)
  if (t) {
    property(key, t, subs, cb, branch)
  } else if (branch.val) {
    refRemoveFieldFinder(branch.val, key, subs, cb)
  }
}

const refCreateFieldFinder = (tree, key, subs, cb) => {
  const branch = tree[key]
  if (branch) {
    remove(subs, cb, branch)
  } else if (tree.val) {
    refCreateFieldFinder(tree.val, key, subs, cb)
  }
}
