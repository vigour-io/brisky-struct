...
const simple = (key, t, subs, cb, tree, removed, transform, diffit) => {
  ...
  // !t means no result
  var result
  if (!removed && t) { // is this fine as default behaviour -- yes
    result = transform(t, subs, tree, key)
  }
  if (!result) {
    if (branch) {
...
// will add string swithc in here lot faster
      ...
      if (key !== 'props' && key !== '_') {
        if (a[key] !== b[key]) {
          if (typeof a[key] === 'function' && typeof b[key] === 'function') {
            console.log('function compare... danger...')
