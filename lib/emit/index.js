...
const override = (t, val, stamp, isNew) => {
  t.stamp = val.stamp
...
const data = (t, val, stamp, isNew) => {
  if (t.stamp !== stamp) {
    t.stamp = (val && typeof val === 'object' && val.stamp !== void 0)
      ? val.stamp
      : stamp
    // what if replace stamp is the same?

    subscription(t, stamp)
    const own = t.emitters && t.emitters.data
    if (own) {
      const struct = own.struct
      console.log('OWN', t.keys())
      fn(t, val, stamp, own)
      if (struct) {
        let i = struct.length
