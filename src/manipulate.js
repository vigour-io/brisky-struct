...
import { reference, removeReference } from './references'
import { getKeys } from './keys'

const create = (t, val, stamp, parent, key, reset, noConflict) => {
  var instance
  const hasType = val &&
    typeof val === 'object' &&
...
const objects = (t, val, stamp, isNew, reset, noConflict) => {
  if (val.stamp) {
    return overrideObjects(t, val, stamp, isNew, reset, noConflict)
  } else if (t.instances) {
    let changed
    for (let key in val) {
      if (key !== 'stamp') {
    ...
    if (type === 'function') {
      ...
      if (!val) {
        ...
        if (val.inherits) {
          ...
          return objects(t, val, stamp, isNew, reset, noConflict)
        }
      }
    } else if (setVal(t, val, stamp)) {
      if (reset) removeAllFields(t, stamp)
